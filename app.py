...
BATCH_SIZE = 10  # Set the batch size for processing
...
def match_jobs():
    ...
    job_urls = search_jobs(location, results=50)  # Adjust to the desired total job count
    ...
    for i in range(0, len(job_urls), BATCH_SIZE):
        ...
        refined_descriptions, job_details = extract_descriptions_and_details_from_urls(batch_urls)
        all_refined_descriptions.extend(refined_descriptions)
        all_job_details.extend(job_details)

    if all_job_details and all_refined_descriptions:
        matching_scores = match_skills(user_skills, user_experiences, all_refined_descriptions)
        result = []

        for i, job in enumerate(all_job_details):
            job_info = {
                'title': job['title'],
                'company': job['company'],
                'location': job['location'],
                'description': all_refined_descriptions[i] if i < len(all_refined_descriptions) else 'N/A',
                'matching_score': matching_scores[i] if i < len(matching_scores) else 0,
                'url': job_urls[i]  # Include URL if needed
            }
            result.append(job_info)

        result_sorted = sorted(result, key=lambda x: x['matching_score'], reverse=True)
